...
export const NewCarBodySchema = Joi.object({
  ...
  engineSize: Joi.number().min(0.0).max(10.0).required(),
  fuelType: JoiFuelType,
  emissionClass: JoiEmissionClass,
  horsepower: Joi.number().integer().min(0).max(500).required(),
  transmission: JoiTransmission,
  numberOfDoors: Joi.number().integer().min(3).max(5).required(),
  numberOfSeats: Joi.number().integer().min(0).max(10).required(),
  bootCapacity: Joi.number().min(0.0).max(700.0).required(),
